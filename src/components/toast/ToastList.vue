<template>
  <TransitionGroup name="slide" tag="div" class="custom-toast-list d-flex flex-column justify-content-end">
    <div 
      v-for="toast in toastStore.list" :key="toast.message.length"
      class="custom-toast overflow-hidden"
      :class="getToastColors(toast.type)"
    >
      <div 
        class="custom-toast-progress-bar" 
        :style="`width: ${(toast.elapsedTime / toast.totalTime) * 100}%`"
      ></div>
...
export default defineComponent({
  ...
  methods: {
    getToastColors(type: ToastType) {
        switch(type) {
            case ToastType.Info: return "text-white bg-primary";
            case ToastType.Success: return "text-white bg-success";
            case ToastType.Warning: return "text-black bg-warning";
            case ToastType.Error: return "text-white bg-danger";
            case ToastType.Exception: return "text-white bg-danger";
            case ToastType.Unauthorized: return "text-white bg-danger";
        }
    },
    getToastIcon(type: ToastType) {
        switch(type) {
            ...
            case ToastType.Error: return "fas fa-times-circle";
            case ToastType.Exception: return "fas fa-bug";
            case ToastType.Unauthorized: return "fas fa-key";
        }
    },
  ...
  mounted() {
    this.stepInterval = setInterval(
        () => this.toastStore.incrementToastTimes(timeStep), 
        timeStep
    ); 
